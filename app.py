...
def init_all():
    ...
    # FIXED: Specifically using the Gemini 3 Flash Preview ID
...
with st.sidebar:
    ...
    # Navigation logic to separate data entry from dashboard
...
if page == "Data Entry":
    ...
    with method[1]:
        st.subheader("📸 AI Scanner (Gemini 3 Flash)")
...
# --- 4. DASHBOARD PAGE ---
    ...
    if not all_df.empty:
        ...
        if pending_df.empty:
            st.info("No active invoices found.")
        else:
            for i, row in pending_df.iterrows():
                with st.expander(f"📋 {row['client_name']} — ${row['amount']} (Due: {row['due_date']})"):
                    ...
                    with c2:
                        ...
                        st.markdown(f'''<a href="{wa_url}" target="_blank"><button style="background-color:#25D366;color:white;width:100%;padding:10px;border-radius:8px;border:none;cursor:pointer;font-weight:bold;">📱 WhatsApp Chat</button></a>''', unsafe_allow_html=True)
